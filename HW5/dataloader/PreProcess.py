...
def img_to_tensor_by_mode(pic: Image.Image) -> torch.Tensor:

    if pic.mode == "I":
        return torch.from_numpy(np.array(pic, np.int32, copy=False))

    elif pic.mode == "I;16":
        return torch.from_numpy(np.array(pic, np.int16, copy=False))

    return torch.ByteTensor(torch.ByteStorage.from_buffer(pic.tobytes()))


def img_to_n_channel(pic: Image.Image) -> int:
...
def ToTensor(pic: Image.Image):
    """handle PIL Image"""

    img = img_to_tensor_by_mode(pic)

    n_channel = img_to_n_channel(pic)
...
def to_var(x, requires_grad: bool = True):
    # if requires_grad:
    ...
    return (
        Variable(x).float()
        if requires_grad
        else Variable(x, requires_grad=requires_grad).float()
    )
...
class PreProcess:
    ...
    def process(self, mask, lms, device="cpu"):
        diff = to_var(
            (
                self.fix.double()
                - torch.tensor(lms.transpose((1, 0)).reshape(-1, 1, 1)).to(self.device)
            ).unsqueeze(0),
            requires_grad=False,
        ).to(self.device)
        ...
        diff = to_var(
            (self.fix.double() - torch.tensor(lms).to(self.device)).unsqueeze(0),
            requires_grad=False,
        ).to(self.device)

        mask_lip = (mask == self.lip_class[0]).float() + (
            mask == self.lip_class[1]
        ).float()
        mask_face = (mask == self.face_class[0]).float() + (
            mask == self.face_class[1]
        ).float()
        ...
        mask_re = F.interpolate(mask_aug, size=self.diff_size).repeat(
            1, diff.shape[1], 1, 1
        )  # (3, 136, 64, 64)
        diff_re = F.interpolate(diff, size=self.diff_size).repeat(
            3, 1, 1, 1
        )  # (3, 136, 64, 64)
        diff_re = diff_re * mask_re  # (3, 136, 32, 32)
        norm = torch.norm(diff_re, dim=1, keepdim=True).repeat(
            1, diff_re.shape[1], 1, 1
        )
    ...
    def __call__(self, image: Image):
        ...
        mask = F.interpolate(
            mask.view(1, 1, 512, 512), (self.img_size, self.img_size), mode="nearest"
        )
