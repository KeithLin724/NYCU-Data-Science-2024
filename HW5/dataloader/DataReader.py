...
class DataReader:
    ...
    def __init__(self, data_dir="data"):
        ...
        self.makeup_names = [
            name.strip() for name in self.data_dir.joinpath(self.makeup).open("rt")
        ]
        self.non_makeup_names = [
            name.strip() for name in self.data_dir.joinpath(self.non_makeup).open("rt")
        ]

        self.random = None

    def read_file(self, name):
        image = Image.open(self.image_dir.joinpath(name).as_posix()).convert("RGB")
        seg = np.asarray(Image.open(self.seg_dir.joinpath(name).as_posix()))
