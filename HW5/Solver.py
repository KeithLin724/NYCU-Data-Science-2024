...
# import net
from module import Generator_makeup, Generator_branch, Discriminator, VGG

from ops.histogram_matching import *
from ops.loss_added import GANLoss


class Solver_makeupGAN:
    ...
    def log_terminal(self):
        ...
        log = f"Elapsed [{elapsed}], Epoch [{self.e + 1}/{self.num_epochs}], Iter [{self.i + 1}/{self.iters_per_epoch}]"
    ...
    def save_models(self):
        ...
        for i in self.cls:
            torch.save(
                getattr(self, "D_" + i).state_dict(),
                os.path.join(
                    self.snapshot_path,
                    "{}_{}_D_".format(self.e + 1, self.i + 1) + i + ".pth",
                ),
            )
    ...
    def load_checkpoint(self):
        self.G.load_state_dict(
            torch.load(
                os.path.join(self.snapshot_path, "{}_G.pth".format(self.checkpoint))
            )
        )
        for i in self.cls:
            getattr(self, "D_" + i).load_state_dict(
                torch.load(
                    os.path.join(
        ...
        for self.e in range(start, self.num_epochs):
            for self.i, (img_A, img_B, mask_A, mask_B) in enumerate(
                ...
                for key_now in self.loss.keys():
                    tl.plot(key_now, self.loss[key_now])
    ...
    def vis_train(self, img_train_list):
        ...
        save_path = os.path.join(
            result_path_train, "{}_{}_fake.jpg".format(self.e, self.i)
        )
    ...
    def vis_test(self):
        ...
        for i, (img_A, img_B) in enumerate(self.data_loader_test):
            ...
            save_path = os.path.join(
                result_path_now, "{}_{}_{}_fake.png".format(self.e, self.i, i + 1)
            )
    ...
    def test(self):
        ...
        for i, (img_A, img_B) in enumerate(self.data_loader_test):
            ...
            save_path_0 = os.path.join(
                result_path_now,
                "{}_{}_{}_fake_single.png".format(self.e, self.i, i + 1),
            )
            save_image(
                self.de_norm(image_list_0.data),
                save_path_0,
                nrow=1,
                padding=0,
                normalize=True,
